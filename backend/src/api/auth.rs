...
pub async fn signup(State(pool): State<PgPool>, Json(payload): Json<NewUser>) -> impl IntoResponse {
    ...
    let res = sqlx::query_as::<_, User>(
        ...
        .fetch_one(&pool)
        .await;
        fn check_user_type_handler(user:&User)->&'static str{
            match user.user_type.as_deref(){
                Some("admin") => "admin",
                Some("client") => "client",
                _ => "general", // Handles `None` and other values
            }
        };
    match res {
        Ok(user) => {
            let token = generate_token(&user.email,check_user_type_handler(&user));
